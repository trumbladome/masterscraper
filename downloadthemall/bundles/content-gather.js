...
function* extractDescriptionInternal(parent) {
...
class Gatherer {
    ...
    *collectSingleSourceInternal(el) {
    ...
    *findTexts() {
        ...
        const set = doc.evaluate("//*[not(ancestor-or-self::a) and " +
            "not(ancestor-or-self::style) and " +
            "not(ancestor-or-self::script)]/text()", doc, null, XPathResult.ORDERED_NODE_ITERATOR_TYPE, null);
    ...
    *findTextLinks() {
        for (const text of this.findTexts()) {
            yield* textlinks_1.getTextLinks(text, true);
        }
    }
    collectTextLinks() {
        if (!this.textLinks) {
    ...
    makeItem(surl, el, title) {
        if (!el.fake && this.selectionOnly &&
    ...
    makeUniqueItemsInternal(arr, known, result) {
        for (const e of arr) {
            if (!e || !e.url) {
                continue;
            }
...
function gather(msg, sender, callback) {
    try {
        ...
        const result = {
            ...
            links: gatherer.makeUniqueItems(Array.from(document.links).map(gatherer.collectLink), gatherer.collectTextLinks()),
            media: gatherer.makeUniqueItems(Array.from(document.querySelectorAll("img")).
                flatMap(gatherer.collectImage), Array.from(document.querySelectorAll("video")).
                flatMap(gatherer.collectMedia), Array.from(document.querySelectorAll("audio")).
...
browser_1.runtime.onMessage.addListener(gather);

})();

/******/ })()
;
//# sourceMappingURL=content-gather.js.map