...
const {createApp} = Vue;
createApp({
  data(){return{ name:'', selector:'', description:'', concurrency:5, retryLimit:3, mask:'*name* -*num*.*ext*', maskPreview:'', autoDetect:true, profiles:{}, recipes:[] }},
  async created(){
    const data = await chrome.storage.sync.get(['recipes','concurrency','mask','autoDetectProfiles','retryLimit']);
